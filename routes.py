...
def index():
    ...
    return render_template('form_submit1.html', error=None)
    #return render_template('form_submit2.html', error=None)
    #return render_template('form_submit3.html', error=None)
...
def results():
    ...
    if model == "CompareNet":
        print(model)
    elif model == "Meta-Learning":
        print(model)
    elif model == "Textual Entailment":
        print(model)
    else:
    ...
    return render_template("results1.html", headlines=[Markup(headline) for headline in headlines], sources=sources, n=n, \
                           veracity=veracity, stances=stances, claim=claim, rep=rep, urls=urls,
                           crowd_stance=crowd_stance, error=res['error'])
